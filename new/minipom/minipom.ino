...
#define highValve   PD_7
#define commonValve PD_5
#define pump        PD_4
...
void setup() {
  ...
  pinMode(highValve, OUTPUT);
  pinMode(commonValve, OUTPUT);
  pinMode(pump, OUTPUT);
  digitalWrite(pump, HIGH);
  digitalWrite(highValve, HIGH);
  digitalWrite(commonValve, HIGH);
...
int perMili,perHarga, stopHargaPompa, valSolRem;
...
void loop() {
 while(1)
 {
  tampilkan(99999,277,6666);
 }
awal:
  lcd.setCursor(0,0);
  lcd.print("fans-electronics");
  lcd.setCursor(0,1);
...
start:
  ...
  valSolRem = eeprom_read_word((uint16_t*)8);
  ...
  if(valueRupiah >= (stopHargaPompa - valSolRem) && valueRupiah < stopHargaPompa && keyPress == true)
  {
    digitalWrite(pump, LOW);
    digitalWrite(highValve, HIGH);// valve besar buka
    digitalWrite(commonValve, LOW);// commond valve aktif
  }
  if(valueRupiah >= stopHargaPompa && keyPress == true)
  {
    ...
    digitalWrite(pump, HIGH);
    digitalWrite(highValve, HIGH);// valve besar buka
    digitalWrite(commonValve, HIGH);// commond valve aktif
    ...
    if(customKey)
    {
      ...
      else if(customKey == '#')
      {
        keyPress = true;
        pompa = true;
        digitalWrite(highValve, LOW);// high valve buka penuh
        digitalWrite(commonValve, HIGH);// commond valve tdk aktif
        digitalWrite(pump, LOW);
        //motor jalan...
      }
      else if(customKey == 'D')
